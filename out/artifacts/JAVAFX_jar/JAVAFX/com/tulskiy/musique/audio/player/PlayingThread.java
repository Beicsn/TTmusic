...
public class PlayingThread extends Actor implements Runnable {
			...
			synchronized (lock) {
				try {
					...
					logger.log(Level.WARNING,
							"Exception while playing. Stopping now", e);
					currentTrack = null;
					stop();
				}
			}
	...
	private boolean openNext() {
		try {
			...
			if (nextEntry.startSample >= 0) {
				currentByte = AudioMath.samplesToBytes(nextEntry.startSample,
						format.getFrameSize());
	...
	private void updatePlaybackTime() {
		if (format != null) {
			playbackTime = AudioMath.bytesToMillis(playbackBytes, format);
		}
		playbackBytes = 0;
	}
	...
	public double getCurrentMillis() {
		if (this.format != null) {
			return AudioMath.bytesToMillis(this.currentByte, this.format);
		}
		return 0.0D;
	}
